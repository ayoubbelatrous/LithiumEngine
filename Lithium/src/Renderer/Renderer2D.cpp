...
namespace Lithium {
	...
	void Renderer2D::Init()
	{
		...
		s_Data.indexbuffer =  CreateRef<IndexBuffer>(s_Data.MaxIndices, quadIndices);
		delete[] quadIndices;
		...
		s_Data.TextureShader = Shader::Load("assets/shaders/main.shader");
	...
	void Renderer2D::BeginScene(const glm::mat4& view, const glm::mat4& transform)
	{
		
		s_Data.projection = view * transform;
	...
	void Renderer2D::Flush()
	{
		...
		int sampler2D[s_Data.MaxTextureSlots];
		for (uint32_t i = 0; i < s_Data.MaxTextureSlots; i++)
		{
			sampler2D[i] = i;
		}
		s_Data.TextureShader->Bind();
		s_Data.TextureShader->SetUniformMat4f("u_projection", s_Data.projection);
		s_Data.TextureShader->SetUniformiv("u_Textures", sampler2D);
		
		s_Data.indexbuffer->Bind();
		RendererCommand::DrawIndexed(s_Data.QuadIndexCount);
